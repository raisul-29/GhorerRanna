...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Security: Ensure no one can register as Admin even if they inject the value
            if user.role == 'Admin':
...
def menu_list(request):
    ...
    # Get all unique categories, normalized to title case to avoid duplicates like "lunch" vs "Lunch"
...
def menu_detail(request, pk):
    ...
    # Count total times this item was delivered
...
def cart_add(request, pk):
    ...
    # Check if redirect is requested (from "Order Now" button)
